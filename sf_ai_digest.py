...
LUMA_SOURCES = ['sf', 'ai-sf', 'nyc', 'ai-nyc', 'la', 'ai-la']
...
NYC_KEYWORDS  = ['new york', 'nyc', 'brooklyn', 'manhattan', 'queens', 'bronx',
                 'new york city', 'lower east side', 'midtown', 'soho', 'tribeca',
                 'williamsburg', 'dumbo', 'fidi', 'flatiron']
LA_KEYWORDS   = ['los angeles', ' la,', ' la ', 'west hollywood', 'santa monica',
                 'culver city', 'venice beach', 'beverly hills', 'hollywood',
                 'downtown la', 'dtla', 'marina del rey', 'playa vista', 'el segundo']

CITY_KEYWORD_MAP = {
    'San Francisco': SF_KEYWORDS,
    'New York City': NYC_KEYWORDS,
    'Los Angeles':   LA_KEYWORDS,
}

def detect_city(location_str: str) -> str:
    """Return canonical city name from a location string, defaulting to SF."""
    loc = (location_str or '').lower()
    for city, kws in CITY_KEYWORD_MAP.items():
        if any(kw in loc for kw in kws):
            return city
    return 'San Francisco'  # fallback for unlabeled Luma events
...
def fetch_cerebral_valley_events() -> list[dict]:
    ...
    # Filter to SF/Bay Area, NYC, or LA + future
    TARGET_KWS = SF_KEYWORDS + NYC_KEYWORDS + LA_KEYWORDS
    now_utc = datetime.now(timezone.utc)
    city_events = []
    for ev in all_events:
        location = (ev.get('location', '') or '').lower()
        venue = (ev.get('venue', '') or '').lower()
        if not any(kw in f"{location} {venue}" for kw in TARGET_KWS):
            continue
        try:
        ...
        city = detect_city(loc_str)
        tz_name = {
            'San Francisco': 'America/Los_Angeles',
            'New York City': 'America/New_York',
            'Los Angeles':   'America/Los_Angeles',
        }.get(city, 'America/Los_Angeles')
        try:
            dt_local = dt.astimezone(pytz.timezone(tz_name))
        ...
        city_events.append({
            ...
            'url': ev.get('url', ''),
            'location': loc_str or city,
            'city': city,
            'source': 'cerebralvalley.ai',
        })
    counts = {c: sum(1 for e in city_events if e['city'] == c) for c in ['San Francisco','New York City','Los Angeles']}
    log.info(f"  → {len(city_events)} Cerebral Valley events: {counts}")
    return city_events
...
def scrape_all_events(force_refresh: bool = False) -> list[dict]:
    ...
    city_counts = {c: sum(1 for e in combined if e.get('city') == c) for c in ['San Francisco','New York City','Los Angeles']}
    log.info(f"Total events after merge + enrichment: {len(combined)} — {city_counts}")
